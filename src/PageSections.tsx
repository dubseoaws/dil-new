...
import { ArrowUpRight, BadgeCheck, ChevronLeft, ChevronRight, Plus, Quote } from 'lucide-react'
...
export function FaqSection() {
  return <section className="section faq-section" id="faq"><div className="container faq-layout"><div><span className="eyebrow"><span />FAQs</span><h2>Dental Implants<br /><em>London FAQs</em></h2><p>Got questions about dental implants in London? Find answers below or visit our <a href="/faq">full FAQ page.</a></p><a className="text-link" href="/faq">Full FAQ page<ArrowUpRight size={17} /></a></div><div className="faq-list">{faqs.map(([question, answer], index) => <details key={question} open={index === 0}><summary>{question}<Plus size={19} /></summary><p>{answer}</p></details>)}</div></div></section>
}
