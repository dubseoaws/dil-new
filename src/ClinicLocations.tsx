  ...
  return <div className="clinic-locations" data-clinic-locations>
    {clinics.map(clinic => <article className="clinic-location" key={clinic.path}>
      ...
      <div className="clinic-location-details">
        <h3><a href={clinic.path}>{clinic.name}<ArrowUpRight size={20} /></a></h3>
        ...
        <div className="clinic-location-actions"><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(clinic.address)}`} target="_blank" rel="noopener noreferrer">Open in Google Maps<ArrowUpRight size={17} /></a><a href={clinic.path}>Visit clinic<ArrowUpRight size={17} /></a></div>
      </div>
    </article>)}
  </div>
}